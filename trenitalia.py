...
def pass1_extract_valid_between(xml_path: Path) -> Tuple[date, date]:
    """Extract ValidBetween FromDate/ToDate from the first CompositeFrame
    encountered. Fallback to a reasonable range if not found.
    """
    from_d = None
    to_d = None
    for _, elem in etree.iterparse(str(xml_path), events=("end",), huge_tree=True):
